...
from codes.Input_Pipeline import make_training_set

from codes.IngrownSegTest import Test_Network
...
def run_main(parameters):
    ...
    #     print("============================= TRAINING is DISABLED =============================")
        ...
        #     # Check if there is saved kf indices
        #     print("=============================", training_parameters['train_test_split']['k_folds'], "FOLD CV=============================")
                                
        #     indices_path = os.path.join(os.path.dirname(input_parameters['output_dir']), 'kf_indices.pkl')
                  
            # if os.path.exists(indices_path):
            #     import pickle
            #     with open(indices_path, 'rb') as f:
            #         kf_indices = pickle.load(f)
            #         print(f"Previously saved k-fold indices loaded successfully!")
            # else:
            #     # Splitting into k folds
            #     kf = KFold(n_splits = int(training_parameters['train_test_split']['k_folds']),shuffle=True)
                
            #     kf_indices = list(kf.split(image_paths))
                
        #         # Save k fold indices
        #         import pickle
        #         with open(indices_path, 'wb') as f:
        #             pickle.dump(kf_indices, f)                    
                    
        #     k_count = 1
            
        #     # Single for loop that works for both modes
        #     for train_idx, test_idx in kf_indices:
        ...
        #     indices_path = os.path.join(os.path.dirname(input_parameters['output_dir']), 'split_indices.pkl')
            
        #     if os.path.exists(indices_path):
        #         import pickle
        #         with open(indices_path, 'rb') as f:
        #             train_idx, val_idx = pickle.load(f)
        #             print(f"Previously saved split indices loaded successfully!")
        #     else:
    ...
    #             modelADDA = TrainingDA_Loop(input_parameters["model_file"], dataset_train, dataset_valid,
    ...
    # elif input_parameters['phase'] == 'test':
        ...
        #     print("=============================== MULTI INPUT IS NOT SUPPORTED ================================")
    ...
    for run in range(run_throughs):
        ...
        _, dataset_test = make_training_set(
            'test',
            None,
            None,
            run_paths,
        ...
        Test_Network(model_file, dataset_test, input_parameters, args=parameters)
    end_time = time.time()
    spent_time = end_time - start_time
    print("Time spent to test is:", int(spent_time // 60), "minutes and", int(spent_time % 60), "seconds")
