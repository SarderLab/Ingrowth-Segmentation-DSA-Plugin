...
from codes.Augmentation_Functions import *
from codes.IngrownSegUtils import resize_special
            ...
            for i, img_name, tar_name in zip(progressbar, self.inputs, self.targets):
                try:
                    ...
                    if type(img_name)==list:
                        ...
                        # Find contours
                        contours, _ = cv2.findContours(tar, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

                        # Create an empty mask to fill contours
                        filled_image = np.zeros_like(tar)

                        # Fill the contours on the mask
                        cv2.drawContours(filled_image, contours, -1, color=255, thickness=cv2.FILLED)

                        # overlay the filled mask on the original tar
                        tar = np.maximum(tar, filled_image) 
